...
class MySerializer(ModelSerializer):
    ...
    class Meta:
        fields = ["equipment_id_id", "n_time", "location", "weather", "temperature", "humidity", "wind_speed", "light",
                  "Pa", "wind_direction","longitude","latitude"]
        model = realtime
...
class DataMonitor2(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        t_data = his.objects.filter(equipment_id_id=equipment_id, y_time__gte=time_point1).aggregate(AQI=Avg('y_AQI'),
                                                                                                     PM10=Avg('y_PM10'),
                                                                                                     PM2_5=Avg(
                                                                                                         'y_PM2_5'),
                                                                                                     O2=Avg('y_O3'),
                                                                                                     ...
                                                                                                     NO2=Avg('y_NO2'),
                                                                                                     CO=Avg('y_CO'))
        y_data = his.objects.filter(equipment_id_id=equipment_id, y_time__range=(time_point2, time_point1)).aggregate(
            y_AQI=Avg('y_AQI'), y_PM10=Avg('y_PM10'), y_PM2_5=Avg('y_PM2_5'), y_O2=Avg('y_O3'), y_SO2=Avg('y_SO2'),
            y_NO2=Avg('y_NO2'), y_CO=Avg('y_CO'))
        ret = {}
        ret.update(t_data)
        ret.update(y_data)
        return HttpResponse(json.dumps(ret))

class DataMonitor3(APIView):
    # authentication_classes = [MyAuthentication,]
    def get(self, request, *args, **kwargs):
        equipment_id = request.GET.get("equipment_id")
        now = datetime.datetime.now()
...
class MiniDataMonitor1(APIView):
    ...
    def get(self, request, *args, **kwargs):
        equipment_id = request.GET.get("equipment_id")
        n_data = realtime.objects.filter(equipment_id=equipment_id).first()

        # pg = MyPagination()
        # pager = pg.paginate_queryset(view=self,request=request,queryset=n_data)
        myser = MySerializer(instance=n_data, many=False)
        return HttpResponse(json.dumps(myser.data))

class MiniDataMonitor2(APIView):
    # authentication_classes = [MyAuthentication,]
    def get(self, request, *args, **kwargs):
        equipment_id = request.GET.get("equipment_id")
        n_data = realtime.objects.filter(equipment_id_id=equipment_id).first()
